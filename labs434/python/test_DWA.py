...
with mujoco.viewer.launch_passive(m, d, key_callback=key_callback) as viewer:
  ...
  while viewer.is_running() and time.time() - start < 1000:
    ...
    if not paused:
            ...
            # main(obstacles,3,4, RobotType.rectangle)
            velocity.ctrl = u[0]
            steering.ctrl = u[1]

            mujoco.mj_step(m, d)  # Update physics simulation
...
# Plot path 1
plt.plot(position_data[:, 0], position_data[:, 1], label='Original Path', marker='o')
